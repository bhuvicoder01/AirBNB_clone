...
const HostDashboard = () => {
  ...
  const { user } = useAuth();
  const [activeTab, setActiveTab] = useState(localStorage.getItem("hostDashboardActiveTab") || "overview");
  ...
  const getHostProperties = async () => {
    try {
      ...
      console.log("Error fetching host properties:", err);
    }
  };

  // Fetch all bookings for host properties
  const getAllBookingsForHostProperties = async () => {
    try {
      const response = await bookingAPI.getAllBookingsForHostProperties(
        user?._id
      );
  ...
  const calculateEarnings = async () => {
    ...
    const lastMonthEarnings = hostBookings
      .filter((booking) => {
        const bookingDate = new Date(booking.checkIn);
  ...
  const fetchHostData = async () => {
    try {
      ...
      await getAllBookingsForHostProperties();
  ...
  // Count bookings for a specific property
  const countBookingForProperty = (propertyId) => {
    return hostBookings.filter((booking) => booking.propertyId === propertyId)
      .length;
  };
        ...
        <ul className="nav nav-tabs mb-4">
          <li className="nav-item">
            <Button
              ...
              onClick={() => {
                localStorage.setItem("hostDashboardActiveTab","overview");
          ...
          <li className="nav-item">
            <Button
              ...
              onClick={() =>{ 
                localStorage.setItem("hostDashboardActiveTab","listings");
          ...
          <li className="nav-item">
            <Button
              ...
              onClick={() =>{ 
                localStorage.setItem("hostDashboardActiveTab","bookings");
          ...
          <li className="nav-item">
            <Button
              ...
              onClick={() => { 
                localStorage.setItem("hostDashboardActiveTab","earnings");
                ...
                <Card shadow>
                  <div className="d-flex justify-content-between align-items-center">
                    <div>
                      ...
                      <h3 className="mb-0">
                        {
                          hostBookings.filter((b) => b.status == "confirmed")
                            .length
                        }
                      </h3>
                    </div>
                ...
                <Card shadow>
                  ...
                  <div className="d-flex gap-2">
                    ...
                    <Button
                      ...
                      fullWidth
                      onClick={() =>
                        handlePropertyAction(property._id, "disable")
                      }
                  ...
                  <span className={`badge mt-3 bg-info`}>
                    {countBookingForProperty(property._id) > 1
                      ? `${countBookingForProperty(property._id)} Bookings`
                      : `${countBookingForProperty(property._id)} Booking`}
                  </span>
                  <span
                    className={`badge mt-3 position-absolute top-0 end-0 ${
                  ...
                  <span
                    className={`badge mt-3 mx-2 text-decoration-none text-dark  ${
                      countBookingForProperty(property._id) > 0
                        ? "bg-warning"
                        : "bg-secondary"
                    }`}
                  >
                    ...
                    <p className="text-muted">
                      {earnings.thisMonth > earnings.lastMonth ? (
                        ...
                        <span className="text-danger">
                          ...
                          {(
                            ((earnings.lastMonth - earnings.thisMonth) /
                              (earnings.lastMonth != 0
                                ? earnings.lastMonth
                                : 1)) *
                            100
